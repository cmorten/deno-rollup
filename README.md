...
```ts
import { rollup } from "https://deno.land/x/drollup@2.41.0+0.16.0/mod.ts";
...
```ts
import { watch } from "https://deno.land/x/drollup@2.41.0+0.16.0/mod.ts";
...
1. Run the deno-rollup `helloDeno` example directly from the repository:
   ...
   ```console
   deno run --allow-read="./" --allow-write="./dist" --allow-net="deno.land" --allow-env --unstable https://deno.land/x/drollup@2.41.0+0.16.0/examples/helloDeno/rollup.build.ts
   ```
