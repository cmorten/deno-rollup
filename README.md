...
```ts
import { rollup } from "https://deno.land/x/drollup@2.38.5+0.13.0/mod.ts";
...
```ts
import { watch } from "https://deno.land/x/drollup@2.38.5+0.13.0/mod.ts";
...
1. Run the deno-rollup `hello-deno` example directly from the repository:
   ...
   ```console
   deno run --allow-read="./" --allow-write="./dist" --allow-net="deno.land" --allow-env --unstable https://deno.land/x/drollup@2.38.5+0.13.0/examples/hello-deno/rollup.build.ts
   ```
